...
describe 'Test TP MetaProgramacion' do
  ...
  describe 'Multithreading' do
    it 'Si se corren dos threads sobre el mismo objeto no se modifica el objeto hasta que terminan' do
      p = Persona.new
      p.edad = 22
      Thread.new{
                  Transactor.perform {
      ...
      expect( p.edad ).to eq ( 22 ) # no se ven las modificaciones hechas en Thread 1 ni en Thread 2

      sleep 5                      # los 2 threads deberían haber terminado
      expect(p.edad).to eq ( 56 )   # thread 2 ganó y pisó los cambios de thread 1
    end
  end
