...
module Transactional
  ...
  module Transaccionable
    ...
    def crearSettersTransaccionales
      ...
      variables.each{ |variable| crearSetterTransaccional "#{variable}".delete("@") }
    end

    def crearSetterTransaccional variableACrear
      define_method "#{variableACrear}=".to_sym do  |parametro|
                            valorViejo = self.instance_variable_get("@#{variableACrear}".to_sym)
                            self.instance_variable_set("@#{variableACrear}".to_sym, parametro)
                            begin
                              Transactor.transaccionActual.guardarCambioDelObjeto self, "@#{variableACrear}".to_sym, valorViejo, parametro
                            rescue
                            end
                          end
    end
